...
public class WalletHelper {
    ...
    public boolean updateBalance(Wallet wallet, boolean sumTotal) {
        ...
        BigInteger miningFee = nestProperties.getMiningEthUnit()
                .multiply(nestProperties.getMiningFeeRate()
                        .multiply(Constant.UNIT_DEC18))
                .toBigInteger();
        ...
        if (sumTotal) {
            ...
            List<PriceSheet> tokenPriceSheets = ethClient.unClosedSheetListOf(address, erc20State.token.getAddress(), minnerState.getMaxFindNum());
            addFreezedAsset(freezedAssetTemp, tokenPriceSheets, erc20State.token);

            // Post2 NToken quotes frozen assets
            if (minnerState.isMustPost2()) {
                List<PriceSheet> nTokenPriceSheets = ethClient.unClosedSheetListOf(address, erc20State.nToken.getAddress(), minnerState.getMaxFindNum());
        ...
        // Should break into contract assets
        BigInteger payableEthAmount = null;
        if (needEth.compareTo(closeEth) > 0) {
            // The required ETH minus the defrosted ETH
            payableEthAmount = needEth.subtract(closeEth);
        } else {
            // Just put in the quotation handling fee
            payableEthAmount = miningFee;
        }

        wallet.setPayableEthAmount(payableEthAmount);
        wallet.setRich(rich);
        return rich;
    }
