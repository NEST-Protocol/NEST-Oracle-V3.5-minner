...
public class MiningServiceImpl implements MiningService {
    ...
    public void closePriceSheets(Wallet wallet) {
        ...
        String closePriceHash = closePriceSheetList(wallet, nonce, token, rich);

        if (!StringUtils.isEmpty(closePriceHash)) {
            nonce = nonce.add(BigInteger.ONE);
            log.info("Tokens unfreeze the hash of asset transactions：{}", closePriceHash);
        }

        if (minnerState.isMustPost2()) {
            Erc20State.Item ntoken = erc20State.nToken;
            String closeNtokenPriceHash = closePriceSheetList(wallet, nonce, ntoken, rich);
            if (closeNtokenPriceHash == null) return;
            log.info("Ntokens unfreeze the hash of asset transactions：{}", closeNtokenPriceHash);
        }
    }

    private String closePriceSheetList(Wallet wallet, BigInteger nonce, Erc20State.Item erc20, boolean rich) {
        // Obtain unthawed quotations
        List<PriceSheet> priceSheets = ethClient.unClosedSheetListOf(wallet.getCredentials().getAddress(), erc20.getAddress(), minnerState.getMaxFindNum());

        // Get quotes that can be defrosted
        List<Uint32> canClosedSheetIndexs = ethClient.canClosedSheetIndexs(priceSheets);
        ...
        if (size1 < minnerState.getCloseMinNum()) {
            ...
            BigInteger lengthOfPriceSheets = ethClient.lengthOfPriceSheets(erc20.getAddress());
            if (lengthOfPriceSheets == null) {
                wait = false;
            ...
            if (size1 > 0 && wait) {
                ...
                BigInteger nowMaxIndex = lengthOfPriceSheets.subtract(BigInteger.ONE);
        ...
        if (size1 == 1) {
            String closeHash = ethClient.close(wallet, nonce, erc20.getAddress(), new Uint256(canClosedSheetIndexs.get(0).getValue()));
            return closeHash;
        }
        String hash = ethClient.closeList(wallet, nonce, erc20.getAddress(), canClosedSheetIndexs);
        return hash;
    }
