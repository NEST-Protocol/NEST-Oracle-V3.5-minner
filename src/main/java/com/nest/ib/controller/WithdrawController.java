...
public class WithdrawController {
    ...
    public R withdrawNtoken(@RequestParam(name = "ethAmount", defaultValue = "0") BigDecimal ethAmount,
                            @RequestParam(name = "nTokenAamount", defaultValue = "0") BigDecimal nTokenAamount,
        ...
        BigInteger eth = MathUtils.decMulInt(ethAmount, Constant.UNIT_INT18).toBigInteger();
        BigInteger token = nTokenAamount.multiply(erc20State.nToken.getDecPowTen()).toBigInteger();
        boolean b = ethClient.withdrawEthAndToken(eth, nTokenAddress, token, WalletHelper.getWallet());
        if (b) {
    ...
    public R withdrawToken(@RequestParam(name = "ethAmount", defaultValue = "0") BigDecimal ethAmount,
                           @RequestParam(name = "tokenAamount", defaultValue = "0") BigDecimal tokenAamount,
        ...
        boolean b = ethClient.withdrawEthAndToken(eth, tokenAddress, token, WalletHelper.getWallet());
        if (b) {
            return R.ok("Transaction sent successfully");
        }
    ...
    public R withdrawNest(@RequestParam(name = "nestAamount") BigDecimal nestAamount) {
        ...
        boolean b = ethClient.withdrawNest(nest, WalletHelper.getWallet());
        if (b) {
            return R.ok("Transaction sent successfully");
        }
